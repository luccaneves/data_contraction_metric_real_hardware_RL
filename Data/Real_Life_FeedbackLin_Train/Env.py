...
class SerialForceEnv(gym.Env):
    ...
    def step(self, action):
        ...
        reward = -(
            ...
            ((action[0]/2 + 0.5)*max_kp ** 2) * self.R1 +
            ((action[1]/2 + 0.5)*max_ki ** 2) * self.R1 +
            ((force_error_norm - self.last_error) ** 2) * self.R2 +
            (((action[0]/2 + 0.5)*max_kp - (self.last_action[0]/2 + 0.5)*max_kp) ** 2) * self.R3 + 
            (((action[1]/2 + 0.5)*max_ki - (self.last_action[1]/2 + 0.5)*max_ki) ** 2) * self.R3
        )
